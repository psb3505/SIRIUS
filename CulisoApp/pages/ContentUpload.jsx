      ...
      {boardData.map((board, index) => (
        <View key={index} style={styles.checkboxContainer}>
          <TouchableOpacity
            ...
            {sendContents.board_id === board.board_id && (
...
const ContentUpload = () => {
  ...
  // 게시판 체크박스 목록 가져오기
  const CheckBoard = () => {
    axios.post(ENDPOINT + 'user/CheckBoard', {}, {
          // 빈 객체 '{}'를 명시적으로 추가
          headers: {
  ...
  const PrevBoardContentsValue = sendContentsNum => {
    ...
    axios
      .post(ENDPOINT + 'user/PrevBoardContentsValue', data, {
        headers: {
          'Content-Type': 'application/json',
          Accept: 'application/json',
        },
  ...
  const handleTopBtnClick = async () => {
    ...
    if (prevPage === 'ContentsComponent') {
      ...
      formData.append('title', changeTitle ? sendContents.title : defaultTitle);
      formData.append('contents', changeContents ? sendContents.contents : defaultContents);
    ...
    try {
      const totalStartTime = Date.now(); // 전체 실행 시간 측정 시작
      const resizeStartTime = Date.now(); // 리사이징 시간 측정 시작
  
      // 새로운 파일만 필터링 (fileUrl이 없는 파일만 리사이징 대상)
      const newFiles = relatedFiles.filter(file => !file.fileUrl);
  
      // 비동기 방식으로 새로운 이미지 리사이징
      const resizedImages = await Promise.all(
        newFiles.map(async (file, index) => {
          ...
          return {
            ...
            type: 'image/webp',
            name: `${originalNameWithoutExt}.webp`,  // 원본 이름에 .webp 확장자 추가
          };

        }),
      );
  
      const resizeEndTime = Date.now(); // 리사이징 시간 측정 종료
      console.log(`Image Resizing Time (Async): ${resizeEndTime - resizeStartTime} ms`);
  
      // 리사이징된 새 이미지 FormData에 추가
      ...
      uniqueDeleteFiles.forEach(file => {
        formData.append('del_img_names', file);
      });
  
      const uploadStartTime = Date.now(); // 업로드 시간 측정 시작
      const path = prevPage === 'ContentsComponent' ? 'ContentsUpdate' : 'ContentsInsert';
      await ContentsControl(path, formData);
      const uploadEndTime = Date.now(); // 업로드 시간 측정 종료
  
      console.log(`Upload Time (Async): ${uploadEndTime - uploadStartTime} ms`);
  
      const totalEndTime = Date.now(); // 전체 실행 시간 측정 종료
      console.log(`Total Execution Time (Async): ${totalEndTime - totalStartTime} ms`);
  
      navigation.navigate('CommunicationMain'); // 즉시 메인 게시판으로 이동
    } catch (error) {
      console.error('Error during image upload (Async):', error);
      Alert.alert('업로드 실패', '게시글 업로드 중 오류가 발생했습니다.');
    }
  };
  ...
  return (
    <CommunityBackground center={false}>
      ...
      <Modal
        isVisible={checkItemsIsOpen}
        onBackdropPress={checkItemsCloseModal}
        style={CustomStyles}>
        <View style={styles.modalContent}>
          <GetImage type="Mark" width={50} height={50} />
